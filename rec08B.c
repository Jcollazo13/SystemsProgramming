...
int main(int argc, char **argv) {
  ...
  if (argc == 3) {
    ...
    // only the parent gets here, close all pipes and wait for 3 children to
    // finish
    close(fd1[0]);
    close(fd1[1]);
    close(fd2[0]);
